...
int GetButton()
{
  ...
  if (btnRow1 >= MAGIC_NUM_LEFT && btnRow1 <= MAGIC_NUM_LEFT + buffer_range) return 0;
  if (btnRow1 >= MAGIC_NUM_MIDDLE && btnRow1 <= MAGIC_NUM_MIDDLE + buffer_range) return 1;
  if (btnRow1 >= MAGIC_NUM_RIGHT && btnRow1 <= MAGIC_NUM_RIGHT + buffer_range) return 2;
  
  if (btnRow2 >= MAGIC_NUM_LEFT && btnRow2 <= MAGIC_NUM_LEFT + buffer_range) return 3;
  if (btnRow2 >= MAGIC_NUM_MIDDLE && btnRow2 <= MAGIC_NUM_MIDDLE + buffer_range) return 4;
  if (btnRow2 >= MAGIC_NUM_RIGHT && btnRow2 <= MAGIC_NUM_RIGHT + buffer_range) return 5;
  
  if (btnRow3 >= MAGIC_NUM_LEFT && btnRow3 <= MAGIC_NUM_LEFT + buffer_range) return 6;
...
void DrawTicTacToe() {
  ...
  for (int i = 0; i < ROWS_NUM; ++i) {
    ...
    for (int j = 0; j < COLS_NUM; ++j) {
      if (boardState[i][j] == PLAYER_BOARD_MARK) {Serial.print("X | ");}
      else if (boardState[i][j] == 2) {Serial.print("O | ");}
      else {Serial.print("  | ");}
